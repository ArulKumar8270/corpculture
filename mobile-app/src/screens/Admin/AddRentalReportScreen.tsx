...
const AddRentalReportScreen = () => {
  ...
  const fetchCompanyRelatedData = async () => {
    try {
      ...
      const { data: productsData } = await axios.get(
        `${process.env.EXPO_PUBLIC_API_URL}/rental-products/getServiceProductsByCompany/${formData.company}`,
        { headers: { Authorization: token || '' } }
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('Rental report payload:', JSON.stringify(payload, null, 2));
      ...
      console.error('Error submitting rental report:', error);
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                return (
                  ...
                  >
                    <Text style={styles.pickerOptionText}>{String(productName)}</Text>
                  </TouchableOpacity>
                );
              }}
            />
...
export default AddRentalReportScreen;
