...
import {
  buildServiceInvoiceGstrWorksheet,
  excelBufferToBase64,
...
const ServiceInvoicesReportScreen = () => {
  ...
  const handleExportExcel = async () => {
    ...
    if (totalCount === 0 && invoices.length === 0) {
    ...
    try {
      setExporting(true);
      const exportLimit = Math.min(Math.max(totalCount || invoices.length, 1), 100000);
      const requestBody = {
        invoiceType: reportType,
        fromDate,
        toDate,
      ...
      const ws = buildServiceInvoiceGstrWorksheet(XLSX, rows);
      const wb = XLSX.utils.book_new();
      const sheetName = reportType === 'quotation' ? 'Service Quotations' : 'Service Invoices';
      XLSX.utils.book_append_sheet(wb, ws, sheetName.slice(0, 31));
      const excelBuffer = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
      const prefix = reportType === 'quotation' ? 'service_quotations' : 'service_invoices';
      const fileName = `${prefix}_gstr_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const handleDownloadInvoicePdf = async (invoice: any) => {
    try {
      const ok = await openServiceInvoicePdf(invoice);
