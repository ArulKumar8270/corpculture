...
const RentalInvoiceListScreen = () => {
  ...
  const onMoveToInvoice = async (entry: any) => {
    try {
      const payload = {
        ...
        // Update invoiceDate when converting quotation -> invoice
        invoiceDate: new Date().toISOString(),
        // Reset send state so invoice can be sent again and new file stored
        invoiceSendStatus: 'NotSent',
        invoiceSentAt: null,
        invoiceLink: [],
      };
  ...
  const onSendInvoice = async (entry: any) => {
    ...
    try {
      ...
      try {
        await axios.put(
          `${getApiBaseUrl()}/rental-payment/${entry._id}`,
          { invoiceSendStatus: 'NotSent', invoiceSentAt: null, invoiceLink: [] },
          { headers: { Authorization: token || '' }, timeout: 30000 }
        );
      } catch {
        // ignore: webhook can still run
      }
  ...
  const onCancelEntry = (entry: any) => {
    if (!entry?._id) return;
    if (entry?.status === 'Cancelled' || entry?.status === 'Paid') return;
        ...
        onPress: async () => {
          try {
            ...
            Toast.show({
              type: 'error',
              text1: 'Error',
              text2: e?.response?.data?.message || 'Failed to cancel invoice',
            });
          }
        },
      },
  ...
  const displayedEntries = listTab === 'cancelled' ? cancelledEntries : activeEntries;
    ...
    return (
      <View style={styles.entryCard}>
        ...
        <View style={styles.actionButtons}>
          ...
          {hasPermission('rentalInvoice', 'edit') && invoiceType !== 'quotation' ? (
            <TouchableOpacity
              style={[styles.actionButton, styles.cancelButton]}
              onPress={() => onCancelEntry(item)}
              disabled={item?.status === 'Cancelled' || item?.status === 'Paid'}
            >
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabRow}>
        <TouchableOpacity
          style={[styles.tabBtn, listTab === 'active' && styles.tabBtnActive]}
          onPress={() => {
            setListTab('active');
            setPage(0);
          }}
        >
          <Text style={[styles.tabBtnText, listTab === 'active' && styles.tabBtnTextActive]}>
            Active ({activeEntries.length})
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.tabBtn, listTab === 'cancelled' && styles.tabBtnActive]}
          onPress={() => {
            setListTab('cancelled');
            setPage(0);
          }}
        >
          <Text style={[styles.tabBtnText, listTab === 'cancelled' && styles.tabBtnTextActive]}>
              ...
              <View style={styles.paginationContainer}>
                <View style={styles.paginationControls}>
                  ...
                  <Text style={styles.paginationText}>
                    Page {page + 1} of {Math.ceil(displayedEntries.length / rowsPerPage) || 1}
                  </Text>
                  <TouchableOpacity
                    style={[
                      styles.paginationButton,
                      page >= Math.ceil(displayedEntries.length / rowsPerPage) - 1 && styles.paginationButtonDisabled
                    ]}
                    onPress={() => handleChangePage(page + 1)}
                    disabled={page >= Math.ceil(displayedEntries.length / rowsPerPage) - 1}
                  >
                    <Text
                      style={[
                        styles.paginationButtonText,
                        page >= Math.ceil(displayedEntries.length / rowsPerPage) - 1 && styles.paginationButtonTextDisabled
                      ]}
                    >
                      Next
                    </Text>
