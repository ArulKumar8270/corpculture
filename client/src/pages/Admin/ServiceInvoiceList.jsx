...
function InvoiceRow(props) {
    ...
    const handleCancelInvoice = async () => {
        if (!hasPermission("serviceInvoice")) {
        ...
        try {
            ...
            console.error("Cancel invoice failed", e);
    ...
    const onMoveToInvoice = async (status) => {
        try {
            ...
            const payload = {
                ...
                // Set invoice date to current date when moving to invoice
...
const ServiceInvoiceList = (props) => {
    ...
    const displayedInvoices = listTab === 1 ? cancelledInvoices : activeInvoices;
            ...
            <>
                ...
                <Paper elevation={3} sx={{ p: 2, borderRadius: '8px', width: '95%' }}>
                    <Tabs value={listTab} onChange={(e, v) => setListTab(v)} sx={{ mb: 1 }}>
                        <Tab label={`Active (${activeInvoices.length})`} />
                        <Tab label={`Cancelled (${cancelledInvoices.length})`} />
                    </Tabs>
                    <TableContainer>
                                ...
                                {displayedInvoices?.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={9} align="center" sx={{ py: 3, color: 'text.secondary' }}>
                                            {listTab === 1 ? "No cancelled invoices found." : "No active invoices found."}
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                    displayedInvoices
