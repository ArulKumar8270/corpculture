...
function RentalInvoiceList(props) {
    ...
    const activeEntries = useMemo(
        () => (filteredRentalEntries || []).filter((e) => e?.status !== "Cancelled"),
    ...
    const displayedEntries = listTab === 1 ? cancelledEntries : activeEntries;
    ...
    const handleCancelEntry = async (entry) => {
        if (!hasPermission("rentalInvoice")) {
            toast.error("You do not have permission.");
            return;
        }
        if (entry?.status === "Cancelled") return;
        if (entry?.status === "Paid") {
            toast.error("Paid invoice cannot be cancelled.");
            return;
        }
        ...
        try {
            ...
            if (res.data?.success) {
                ...
                toast.error(res.data?.message || "Failed to cancel invoice");
            }
        } catch (e) {
            console.error("Cancel rental invoice failed", e);
    ...
    const onMoveToInvoice = async (status, entry) => {
        try {
            ...
            const payload = {
                ...
                // Set invoice date to current date when moving quotation -> invoice
                invoiceDate: new Date().toISOString(),
                // Reset send status/links so invoice can be sent again after conversion
    ...
    return (
        <Box sx={{ p: 3, bgcolor: 'background.default', minHeight: '100vh', width: '87%' }}>
            ...
            <Paper elevation={3} sx={{ p: 2, borderRadius: '8px' }}>
                <Tabs value={listTab} onChange={(e, v) => setListTab(v)} sx={{ mb: 1 }}>
                    <Tab label={`Active (${activeEntries.length})`} />
                    <Tab label={`Cancelled (${cancelledEntries.length})`} />
                </Tabs>
                                                    ...
                                                    <Stack spacing={1}>
                                                        ...
                                                        {hasPermission("rentalInvoice") ? (
                                                            <Button
                                                                ...
                                                                color="error"
                                                                size="small"
                                                                disabled={entry?.status === "Cancelled" || entry?.status === "Paid"}
